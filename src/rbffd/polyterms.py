        ...
        for i in range(pdeg+1):
            for j in range(pdeg+1-i):
                for k in range(pdeg+1-i-j):
                    p[idx] = args[2]**i * args[1]**j * args[0]**k
                    idx += 1
...
def jax_binom(n : int , k : int):
    ...
    return np.int32(n * jax_binom(n-1,k-1) / k)


@partial(jax.jit, static_argnums=(1,2))
def polyterms_jax(points, pdeg : int, mult_term=1):
    ...
    for i in range(max_range):
        if ndims > 1:
            ...
            poly = poly.at[:,index:index+terms].set(polyterms_jax(points[:,0:-1], pdeg-i, mult_term * points[:,-1]**i))
        else:
            terms = 1
